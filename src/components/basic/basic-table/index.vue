...
<template>
	...
	<div class="card table-main">
		...
		>
			...
			<template v-for="item in tableColumns" :key="item">
				...
				<!-- other 循环递归 -->
				<basic-table-column v-if="!item.type && item.prop && item.isShow" :column="item">
			...
			<template #empty>
				<div class="table-empty">
					<slot name="empty">
						<img src="@/assets/images/common/no-data.png" style="height: 400px" alt="notData" />
						<div>暂无数据</div>
					</slot>
				</div>
			</template>
	...
	<!-- 列设置 -->
	<basic-col-setting v-if="toolButton" ref="colRef" v-model:colSetting="colSetting" />
</template>
...
import { BreakPoint, ColumnProps } from "@/components/basic/types";
import { ElTable, TableProps } from "element-plus";
import { Refresh, Printer, Operation, Search } from "@element-plus/icons-vue";
import { filterEnum, formatValue, handleProp, handleRowAccordingToProp } from "@/utils/index";
import { BasicSearchForm, BasicTableColumn, BasicColSetting, BasicPagination } from "@/components/basic/index";

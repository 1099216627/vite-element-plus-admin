...
<script lang="tsx" setup name="BasicTableColumn">
				...
				>
					{{
						...
						header: () => {
							...
							if (slots[`${propToCamel(item.prop!)}Header`]) return slots[`${propToCamel(item.prop!)}Header`]!({ row: item });
							return item.label;
						}
					}}
				</el-table-column>
